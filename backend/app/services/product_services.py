...
class ProductService:
    ...
    def get_all(self) -> List[ProductResponseSchema]:
        ...
        return ProductListResponseSchema(products=product_response,total=len(products))

    def get_product_by_id(self, product_id: int) -> ProductResponseSchema:
        product = self.product_repository.get_by_id(product_id)
        if not product:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, data=f"Product with id {product_id} not found")
        return ProductResponseSchema.model_validate(product)

    def get_product_by_category(self, category_id: int) -> ProductResponseSchema:
        category=self.category_repository.get_by_id(category_id)

        if not category:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, data=f"Category with id {category_id} not found")
    ...
    def create_product(self, product_data: ProductCreateSchema) -> ProductResponseSchema:
        ...
        if not category:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, data=f"Category with id {category.id} not found")
